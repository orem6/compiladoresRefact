...
public class CqlLexer {
    ...
    public List<NoSqlToken> tokenize() {
        ...
        while (pos < input.length()) {
            ...
            if (Character.isDigit(c) || (c == '-' && pos + 1 < input.length() && Character.isDigit(input.charAt(pos + 1)))) { readNumber(); continue; }
    ...
    private void readBlockComment() {
        ...
        while (pos + 1 < input.length()) {
            if (input.charAt(pos) == '*' && input.charAt(pos + 1) == '/') { sb.append("*/"); pos += 2; col += 2; tokens.add(new NoSqlToken(NoSqlTokenType.COMMENT, sb.toString(), sl, sc)); return; }
            if (input.charAt(pos) == '\n') { line++; col = 0; }
            sb.append(input.charAt(pos)); pos++; col++;
        }
    ...
    private void readString() {
        ...
        while (pos < input.length()) {
            char c = input.charAt(pos);
            if (c == '\'') { sb.append("'"); pos++; col++; tokens.add(new NoSqlToken(NoSqlTokenType.STRING, sb.toString(), sl, sc)); return; }
            if (c == '\n') { errors.add(new NoSqlSyntaxError("CQL_UNCLOSED_STRING", "String sin cerrar.", sl, sc, sb.toString(), "LEXICAL")); return; }
            sb.append(c); pos++; col++;
        }
    ...
    private void readNumber() {
        ...
        while (pos < input.length() && Character.isDigit(input.charAt(pos))) { sb.append(input.charAt(pos)); pos++; col++; }
    ...
    private void readIdentifier() {
        ...
        if (KEYWORDS.contains(upper)) { tokens.add(new NoSqlToken(NoSqlTokenType.KEYWORD, word, sl, sc)); }
        else if (DATA_TYPES.contains(upper)) { tokens.add(new NoSqlToken(NoSqlTokenType.KEYWORD, word, sl, sc)); }
        else if (upper.equals("TRUE") || upper.equals("FALSE")) { tokens.add(new NoSqlToken(NoSqlTokenType.BOOLEAN, word, sl, sc)); }
        else if (upper.equals("NULL")) { tokens.add(new NoSqlToken(NoSqlTokenType.NULL, word, sl, sc)); }
        else { tokens.add(new NoSqlToken(NoSqlTokenType.IDENTIFIER, word, sl, sc)); }
    }
}
