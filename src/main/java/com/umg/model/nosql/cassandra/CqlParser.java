...
public class CqlParser {
    ...
    private static final Set<String> WHERE_STOP_WORDS = Set.of("ALLOW", "ORDER", "LIMIT", "IF");
    ...
    public NoSqlAnalysisResult parse(List<NoSqlToken> inputTokens) {
        ...
        if (tokens.isEmpty()) {
            addError("CQL_EMPTY", "Sentencia CQL vacia.", 0, 0, "");
            return result;
        }
        ...
        switch (kw) {
            ...
            case "USE" -> parseUse();
            default -> addError("CQL_UNSUPPORTED_STATEMENT", "Sentencia CQL no soportada: " + first.getLexeme() + ".", first);
        }
    ...
    private NoSqlToken skipCommentsAndSemicolons() {
        while (pos < tokens.size()) {
            ...
            if (t.getType() == NoSqlTokenType.COMMENT || t.getType() == NoSqlTokenType.SEMICOLON) { pos++; }
            else { return tokens.get(pos); }
        }
    ...
    private void parseSelect() {
        ...
        parseTableName();
        if (current().getLexeme().equalsIgnoreCase("WHERE")) { advance(); result.addDetectedClause("WHERE"); skipUntilClauseOrSemicolon(); }
        if (current().getLexeme().equalsIgnoreCase("ALLOW")) { advance(); expectKeyword("FILTERING", "CQL_EXPECTED_FILTERING", "Se esperaba FILTERING."); result.addDetectedClause("ALLOW_FILTERING"); }
        if (current().getLexeme().equalsIgnoreCase("ORDER")) { advance(); expectKeyword("BY", "CQL_EXPECTED_BY", "Se esperaba BY."); result.addDetectedClause("ORDER_BY"); skipUntilSemicolon(); }
        if (current().getLexeme().equalsIgnoreCase("LIMIT")) { advance(); result.addDetectedClause("LIMIT"); if (current().getType() == NoSqlTokenType.NUMBER) advance(); }
    }
    ...
    private void parseInsert() {
        ...
        advance();
        expectKeyword("INTO", "CQL_EXPECTED_INTO", "Se esperaba INTO.");
        result.addDetectedClause("INTO");
        parseTableName();
        if (current().getType() == NoSqlTokenType.LEFT_PAREN) { advance(); parseIdentifierList(); if (current().getType() == NoSqlTokenType.RIGHT_PAREN) advance(); }
        expectKeyword("VALUES", "CQL_EXPECTED_VALUES", "Se esperaba VALUES.");
        result.addDetectedClause("VALUES");
        if (current().getType() == NoSqlTokenType.LEFT_PAREN) { advance(); skipUntilRightParen(); }
    }
    ...
    private void parseUpdate() {
        ...
        advance();
        parseTableName();
        if (current().getLexeme().equalsIgnoreCase("USING")) { advance(); result.addDetectedClause("USING"); skipUntilKeyword("SET"); }
        ...
        if (current().getLexeme().equalsIgnoreCase("IF")) { advance(); result.addDetectedClause("IF"); skipUntilSemicolon(); }
    }
    ...
    private void parseDelete() {
        ...
        advance();
        NoSqlToken next = current();
        if (next.getType() == NoSqlTokenType.IDENTIFIER) { parseIdentifierList(); }
    ...
    private void parseCreate() {
        ...
        if (next.equals("KEYSPACE")) { parseCreateKeyspace(); }
        else if (next.equals("TABLE")) { parseCreateTable(); }
        else { addError("CQL_UNSUPPORTED_STATEMENT", "CREATE debe ser KEYSPACE o TABLE.", current()); }
    }
    ...
    private void parseCreateKeyspace() {
        ...
        advance();
        if (current().getLexeme().equalsIgnoreCase("IF")) { advance(); expectKeyword("NOT", "CQL_EXPECTED_NOT", ""); expectKeyword("EXISTS", "CQL_EXPECTED_EXISTS", ""); result.addDetectedClause("IF_NOT_EXISTS"); }
        ...
        if (current().getLexeme().equalsIgnoreCase("WITH")) { advance(); result.addDetectedClause("WITH"); skipUntilSemicolon(); }
    }
    ...
    private void parseCreateTable() {
        ...
        if (current().getType() == NoSqlTokenType.LEFT_PAREN) {
            ...
            while (pos < tokens.size() && parenLevel > 0) {
                ...
                if (t.getLexeme().equalsIgnoreCase("PRIMARY")) { result.addDetectedClause("PRIMARY_KEY"); }
            }
        } else {
    ...
    private void parseAlter() {
        ...
        if (current().getLexeme().equalsIgnoreCase("TABLE")) {
            ...
            if (current().getLexeme().equalsIgnoreCase("ADD") || current().getLexeme().equalsIgnoreCase("DROP") || current().getLexeme().equalsIgnoreCase("RENAME")) {
                String op = current().getLexeme().toUpperCase();
                advance();
                result.addDetectedClause(op);
                skipUntilSemicolon();
            }
        } else { addError("CQL_UNSUPPORTED_STATEMENT", "ALTER debe ser TABLE.", current()); }
    }
    ...
    private void parseDrop() {
        ...
        if (what.equals("TABLE") || what.equals("KEYSPACE")) {
            result.setStatementType(what.equals("TABLE") ? "CASSANDRA_DROP_TABLE" : "CASSANDRA_DROP_KEYSPACE");
            result.addDetectedClause("DROP_" + what);
            advance();
            if (current().getLexeme().equalsIgnoreCase("IF")) { advance(); expectKeyword("EXISTS", "CQL_EXPECTED_EXISTS", ""); }
            if (current().getType() == NoSqlTokenType.IDENTIFIER) advance();
            else addError("CQL_EXPECTED_NAME", "Se esperaba nombre.", current());
        } else { addError("CQL_UNSUPPORTED_STATEMENT", "DROP debe ser TABLE o KEYSPACE.", current()); }
    }
    ...
    private void parseTruncate() {
        ...
        advance();
        if (current().getLexeme().equalsIgnoreCase("TABLE")) { advance(); }
        parseTableName();
    }
    ...
    private void parseTableName() {
        if (current().getType() == NoSqlTokenType.IDENTIFIER) advance();
        else addError("CQL_EXPECTED_TABLE", "Se esperaba nombre de tabla/keyspace.", current());
    }
    ...
    private void parseIdentifierList() {
        ...
        while (pos < tokens.size()) {
            ...
            if (t.getType() == NoSqlTokenType.IDENTIFIER || (t.getType() == NoSqlTokenType.OPERATOR && "*".equals(t.getLexeme()))) { advance(); first = false; }
    ...
    private void expectKeyword(String keyword, String errorCode, String message) {
        if (pos < tokens.size() && current().getLexeme().equalsIgnoreCase(keyword)) { advance(); }
        else if (!message.isEmpty()) { addError(errorCode, message, current()); }
    }
