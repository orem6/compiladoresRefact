...
public class MongoParser {
    ...
    public NoSqlAnalysisResult parse(List<NoSqlToken> inputTokens) {
        ...
        if (methodToken.getType() != NoSqlTokenType.KEYWORD || !SUPPORTED_METHODS.contains(methodToken.getLexeme())) {
            addError("MONGO_UNSUPPORTED_METHOD", "Metodo MongoDB no soportado: " + methodToken.getLexeme(), methodToken);
            return result;
        }
        ...
        if (expectedMin != null && argsParsed < expectedMin) {
            addError("MONGO_INVALID_ARGUMENT_COUNT", "El metodo " + methodName + " requiere al menos " + expectedMin + " argumento(s).", methodToken);
        }

        if (pos < tokens.size() && current().getType() == NoSqlTokenType.RIGHT_PAREN) { advance(); }
        if (pos < tokens.size() && current().getType() == NoSqlTokenType.SEMICOLON) { advance(); }

        if (!braceStack.isEmpty()) {
            addError("MONGO_UNBALANCED_BRACES", "Llaves/corchetes/parentesis desbalanceados. Faltan " + braceStack.size() + " cierre(s).", tokens.get(tokens.size() - 1));
        }
    ...
    private int parseArguments() {
        int count = 0;
        while (pos < tokens.size() && current().getType() != NoSqlTokenType.RIGHT_PAREN && current().getType() != NoSqlTokenType.EOF) {
            if (current().getType() == NoSqlTokenType.LEFT_BRACE) { parseObject(); count++; }
            else if (current().getType() == NoSqlTokenType.LEFT_BRACKET) { parseArray(); count++; }
            else { break; }
            if (pos < tokens.size() && current().getType() == NoSqlTokenType.COMMA) { advance(); }
        }
        return count;
    }
    ...
    private void parseObject() {
        ...
        advance();
        boolean first = true;
        while (pos < tokens.size() && current().getType() != NoSqlTokenType.RIGHT_BRACE && current().getType() != NoSqlTokenType.EOF) {
            if (!first) {
                if (current().getType() == NoSqlTokenType.COMMA) { advance(); }
                else { addError("MONGO_INVALID_OBJECT_SYNTAX", "Se esperaba ',' entre pares del objeto.", current()); break; }
            }
            first = false;
            if (current().getType() != NoSqlTokenType.IDENTIFIER && current().getType() != NoSqlTokenType.STRING && current().getType() != NoSqlTokenType.MONGO_OPERATOR) {
                addError("MONGO_INVALID_OBJECT_SYNTAX", "Se esperaba clave en el objeto.", current()); break;
            }
            advance();
            if (current().getType() != NoSqlTokenType.COLON) { addError("MONGO_INVALID_OBJECT_SYNTAX", "Se esperaba ':' despues de la clave.", current()); break; }
            advance();
            parseValue();
        }
        if (pos < tokens.size() && current().getType() == NoSqlTokenType.RIGHT_BRACE) { advance(); if (!braceStack.isEmpty() && braceStack.peek() == '{') braceStack.pop(); }
    }
    ...
    private void parseArray() {
        ...
        advance();
        boolean first = true;
        while (pos < tokens.size() && current().getType() != NoSqlTokenType.RIGHT_BRACKET && current().getType() != NoSqlTokenType.EOF) {
            if (!first) {
                if (current().getType() == NoSqlTokenType.COMMA) { advance(); }
                else { addError("MONGO_INVALID_ARRAY_SYNTAX", "Se esperaba ',' entre elementos.", current()); break; }
            }
            first = false;
            parseValue();
        }
        if (pos < tokens.size() && current().getType() == NoSqlTokenType.RIGHT_BRACKET) { advance(); if (!braceStack.isEmpty() && braceStack.peek() == '[') braceStack.pop(); }
    }
    ...
    private void parseValue() {
        ...
        switch (current().getType()) {
            ...
            default -> { addError("MONGO_INVALID_VALUE", "Valor invalido.", current()); advance(); }
        }
    }
    ...
    private void expectToken(NoSqlTokenType expectedType, String errorCode, String message) {
        if (pos < tokens.size() && tokens.get(pos).getType() == expectedType) { advance(); }
        else { addError(errorCode, message, current()); }
    }
